    ...
    return (
        <ThemeProvider>
            ...
            <div className={`flex flex-col ${isDarkMode ? 'bg-gray-900 text-black' : 'bg-gray-100 text-gray-900'} transition-colors duration-300`}>
                ...
                <main className="flex-1 container mx-auto p-4 overflow-y-auto hidden-scrollbar flex items-center justify-center py-10">
                    <div className="w-full max-w-lg animate-fade-in">
                        <div className="text-center mb-8">
                            <h2 className={`text-3xl font-bold tracking-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Create an account</h2>
                            <p className={`mt-2 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Start managing your tasks effectively today.</p>
                        </div>
                        ...
                        <div className={`p-8 space-y-6 rounded-2xl shadow-xl border ${isDarkMode ? 'bg-gray-800/50 border-gray-700 backdrop-blur-sm' : 'bg-white border-gray-100'}`}>
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div>
                                    <label className={`block text-sm font-medium mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Gender</label>
                                    <div className="flex items-center space-x-6">
                                        {['Male', 'Female', 'Other'].map((g) => (
                                            <label key={g} className="inline-flex items-center cursor-pointer">
                                                ...
                                                />
                                                <span className={`ml-2 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{g}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                {inProgress ?
                                    <button
                                        disabled
                                        className="w-full px-4 py-2.5 font-medium text-white bg-indigo-500/70 rounded-lg cursor-not-allowed flex justify-center items-center mt-2"
                                    >
                                        <PropagateLoader size={8} color="white" className="mb-2" />
                                    </button> :
                                    <button
                                        type="submit"
                                        className="w-full px-4 py-2.5 font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all shadow-sm hover:shadow mt-2"
                                    >
                                        Create Account
                                    </button>
                                }
                            ...
                            <p className={`text-center text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                                Already have an account? <Link className="font-semibold text-indigo-600 hover:text-indigo-500 transition-colors" to={"/signIn"}>Sign in</Link>
                            </p>
                        </div>
                    </div>
                </main>
